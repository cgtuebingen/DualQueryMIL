...
class Merger(nn.Module):
    def __init__(self, proj_dim):
        ...
        self.dino_head = nn.Linear(2048, proj_dim)
        self.dino_v2_head = nn.Linear(1024, proj_dim)

    def forward(self, data):
        dino_out = self.dino_head(data['dino'])
        dino_v2_out = self.dino_v2_head(data['dino_v2'])
        swav_out = self.swav_head(data['swav'])
        joint = torch.cat([dino_out, dino_v2_out, swav_out], dim=-1)
        return joint
...
class Perceiver(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.merge_embeddings = Merger(input_channels//3)
...
if __name__ == "__main__":
    ...
    model = Perceiver(input_channels=512,
                      input_axis=1,
                      num_freq_bands=6,
                      max_freq=10.,
                      depth=6,
